...
if (isset($_SESSION['idCliente'])) {
    if(isset($_REQUEST['guardar'])){
        ...
        $resCli=mysqli_query($conectar,$queryCli);
        ...
        $resRec=mysqli_query($conectar,$queryRec);
        if($resCli && $resRec){
            echo '<meta http-equiv="refresh" content="0; url=cliente.php?modulo=pasarela" />';
        }
        else{
        ?>
                ...
                <div class="col-12">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="emailCli" id="emailCli" class="form-control"
                         readonly="readonly" value="<?php echo $rowCli['email'] ?>">
                    </div>
                    <div class="form-group">
                        <label for="">Direccion</label>
                        <textarea name="direccionCli" id="direccionCli" class="form-control" row="3">
                            <?php echo $rowCli['direccion'] ?></textarea>
                    </div>
                </div>
                <div class="col-12">
                    ...
                    <div class="form-group">
                        <label for="">Direccion</label>
                        <textarea name="direccionRec" id="direccionRec" class="form-control" row="3"
                           value=" <?php echo $rowRec['direccion'] ?>" required="required" ></textarea>
                    </div>
                    <div class="form-check">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" id="jalar">
                            Recojer datos del cliente
                        </label>
                    </div>
                </div>
            </div>
